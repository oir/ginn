...
class NonlinOp {
  public : virtual bool backward_requires_input() const {// failsafe default
                                                         return true;}
...
class IdentityOp : public NonlinOp<Scalar, Kind> {
  public : bool backward_requires_input() const override{return false;}
...
//                 bool accumulate) const override {
//     add_or_assign(
//         dx, accumulate, dy.t() * (y.t() > Scalar(0)).template
//         cast<Scalar>());
//   }
//
...
//
//   void forward(Tensor<Scalar>& y, const Tensor<Scalar>& x) const override {
//     // Disabling log(0) == -inf because grad would be inf (currently 0-div
//     err) GINN_ASSERT(make_bool(x.dev(), (x.t() > Scalar(0)).all()),
