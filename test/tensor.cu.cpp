...
#define GPU_ASSIGNED_TENSOR(t, ...)                                            \
  TensorType t_(__VA_ARGS__);                                                  \
  using GpuScalar = typename TensorType::Scalar::template To<GPU>;             \
  Tensor<GpuScalar> t(gpu());                                                  \
  t = t_
#define CPU_ASSIGNED_TENSOR(t, ...)                                            \
  TensorType t_(__VA_ARGS__);                                                  \
  using CpuScalar = typename TensorType::Scalar::template To<CPU>;             \
  Tensor<CpuScalar> t(cpu());                                                  \
  t = t_

// Take one of the tensor makers above and test it with this body
#define TEST_CTOR(TENSOR_CTOR, name, INIT_DEV_KIND, DEV_KIND)                  \
  TEMPLATE_TEST_CASE(name, "[tensor]", Tensor<Float<INIT_DEV_KIND>>) {         \
    using TensorType = TestType;                                               \
    using Scalar = typename TensorType::Scalar;                                \
    using RawScalar = typename Scalar::Raw;                                    \
    auto init_dev = default_dev<INIT_DEV_KIND>();                              \
                                                                               \
    SECTION("Device") {                                                        \
    ...
    SECTION("Shape") {                                                         \
      ...
      CHECK(t.shape().size() == 3);                                            \
                                                                               ...
                                                                               \
    SECTION("Value") {                                                         \
      static_assert(std::is_same_v<RawScalar, float>,                          \
                    "Unexpected Scalar type!");                                \
      RawScalar val;                                                           \
      if constexpr (std::is_same_v<RawScalar, float>) { val = 0.6; }           \
      TENSOR_CTOR(t, init_dev, {2, 1, 3}, val);                                \
      CHECK(t.dev()->kind() == DEV_KIND);                                      \
      CHECK(t.size() == 6);                                                    \
      CHECK(t.shape().size() == 3);                                            \
      auto cpu_t = t.copy_to(cpu());                                           \
      for (Size i = 0; i < t.size(); i++) { CHECK(cpu_t.v()[i] == val); }      \
    }                                                                          \
  }
...
//
//   SECTION("Constant") {
//     static_assert(std::is_same_v<Scalar, Real> or std::is_same_v<Scalar, Int>
//     or
