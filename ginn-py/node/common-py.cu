...
void bind_common_nodes_gpu(py::module_& m) {
  ...
  for_each<Real, /*Half,*/ Int>([&](auto scalar) {
    ...
    PyNode<Scalar, GPU, AddNode>(m, name<Scalar, GPU>("AddNode"));
    ...
    // PyNode<Scalar, SubtractScalarNode>(m,
    // name<Scalar>("SubtractScalarNode")); m.def("SubtractScalar",
    // &SubtractScalar<Real, Np>); m.def("SubtractScalar", &SubtractScalar<Int,
    // Np>);
